...
public static class ServiceCollectionExtensions
{
    ...
    extension(IServiceCollection services)
    {
        ...
        public IServiceCollection AddConduit(ILog? logger = null)
        {
        ...
        /// <param name="logger">A logger instance to use, if not provided the default console logger will be used.</param>
        /// <returns>The <see cref="IServiceCollection"/> so that additional calls can be chained.</returns>
        public IServiceCollection AddConduit(Action<IConduitConfigurationBuilder> configure,
            ILog? logger = null)
        {
        ...
        /// <returns>The <see cref="IServiceCollection"/> so that additional calls can be chained.</returns>
        internal IServiceCollection AddRange(params ServiceDescriptor[] defs)
        {
            foreach(var def in defs) services.Add(def);
            return services;
        }
    }
}